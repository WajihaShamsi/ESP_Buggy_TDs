#include "mbed.h"
#include "QEI.h"
#include <cmath>
#include "Serial.h"
Serial pc(USBTX, USBRX);
Serial hm10(PA_11, PA_12);   // PA_11=TX, PA_12=RX
DigitalOut Bluetooth_power(PC_11);

DigitalOut bipR(PC_10);
DigitalOut bipL(PC_12);


/*======================== SENSOR HARDWARE ========================*/
AnalogIn sensors[6] = {A0, A1, A2, A3, A4, A5};
DigitalOut darlington[6]={D8,D7,D6,D5,D4,D3};
float sensor_values[6];

// ── Hardcode  measured black/white values  ──  
const float min_vals[6] = {0.136f, 0.106f, 0.131f, 0.134f, 0.131f, 0.100f};  // black
const float max_vals[6] = {0.932f, 0.925f, 0.930F, 0.930f, 0.928f, 0.902f};  // white
float range[6]   = {0};

/*======================== LINE PID GAINS ========================*/
float line_Kp = 1.45f;
float line_Ki = 0.0f;
float line_Kd = 2.0f;

float base_speed_ms = 1.6f;

/*======================== LINE PID STATE ========================*/
float error_val  = 0.0f;
float last_error = 0.0f;
float integral   = 0.0f;
float derivative = 0.0f;
float correction = 0.0f;

volatile float target_speed_L = 0.0f;
volatile float target_speed_R = 0.0f;

int lost_counter = 0;

/*======================== EOL LATCH SYSTEM ========================*/
int eol_counter = 0;
const int EOL_CONFIRM = 0;

int centered_count = 0;
const int CENTRED_THRESHOLD = 10;
//bool eol_active = false;

int center_counter = 0;
const int CENTER_CONFIRM = 3;

volatile int snap_eol_L = 0;
volatile int snap_eol_R = 0;

float STOP_DISTANCE_MM = 150.0f;
/*======================== SPEED CONSTANTS ========================*/
float SAMPLE_TIME   = 0.006f;
int   PPR           = 256;
int   GEAR_RATIO    = 15;
float WHEEL_RADIUS  = 0.0394f;
float PI            = 3.1415926f;
float WHEEL_BASE    = 0.20f;
float PWM_FREQ_HZ   = 20000.0f;
float WHEEL_CIRC    = 2.0f * 3.1415926f * 0.0394f;
float COUNTS_PER_WHEEL_REV = 256 * 4;
float COUNTS_PER_M  = (256 * 4) / (2.0f * 3.1415926f * 0.0394f);

/* 180 5DEGREE TURN (ENCODER BASED) */
float COUNTS_180 = (COUNTS_PER_M * (PI * WHEEL_BASE/2)+280);


float Kp_L = 0.8f;
float Kp_R = 0.8f;
float Ki   = 0.0f;
float Kd   = 1.1f;
/*======================== EMERGENCY ========================*/
volatile bool emergency_stop = false;

/*======================== VELOCITY GLOBALS ========================*/
volatile int   dL_ticks = 0,  dR_ticks = 0;
volatile float tickRateL = 0.0f, tickRateR = 0.0f;
volatile float vL = 0.0f,  vR = 0.0f;
volatile float uL = 0.5f,  uR = 0.5f;
volatile float eL = 0.0f,  eR = 0.0f;
volatile float uL_prev = 0.5f, uR_prev = 0.5f;
volatile float eL_prev = 0.0f, eR_prev = 0.0f;
volatile float eL_prev2 = 0.0f, eR_prev2 = 0.0f;
volatile float v_robot = 0.0f, w_robot = 0.0f;

/*======================== HARDWARE ========================*/
QEI left_encoder(PB_14, PB_15, NC, PPR, QEI::X4_ENCODING);
QEI right_encoder(PB_2,  PB_1,  NC, PPR, QEI::X4_ENCODING);

PwmOut pwmL(PC_8);
PwmOut pwmR(PC_6);

DigitalOut enable(PC_4);
DigitalOut dirL(PA_13);
DigitalOut dirR(PB_12);

Ticker speedTicker;
Ticker line_ticker;

volatile int last_tick_left = 0, last_tick_right = 0;

/*======================== HELPERS ========================*/
float EnsureSafe(float x, float min_val, float max_val) {
    if (x < min_val) return min_val;
    if (x > max_val) return max_val;
    return x;
}

static inline float tickRateToVel(float tick_rate, float counts_per_m) {
    return tick_rate / counts_per_m;
}

/*======================== MOTOR STOP ========================*/
void stop_motors() {
    pwmL.write(0.5f);
    pwmR.write(0.5f);
}

/*======================== FSM ========================*/
enum State { FOLLOWING, LOST, STOPPED, TURNING, BRAKING, BOOST, SLOW, INCLINE};
State current_state = STOPPED;
State prev_state = STOPPED;

/*======================== SENSOR FUNCTIONS ========================*/
void read_sensors() {
    for (int i = 0; i < 6; i++) {
        //float raw  = sensors[i].read();
        //float norm = 0.0f;
        //if (range[i] > 0.0001f)
        //    norm = (raw - min_vals[i]) / range[i];
        //if (norm < 0.0f) norm = 0.0f;
        //if (norm > 1.0f) norm = 1.0f;
        //sensor_values[i] = norm;
        for (int j = 0; j < 6; j++) darlington[j] = 0;
        wait_us(100);
        float ambient = sensors[i].read();  // 纯环境光

        // 只开第 i 个
        darlington[i] = 1;
        wait_us(100);
        float active = sensors[i].read();

        float raw = active - ambient;        // 抵消环境光
        if (raw < 0.0f) raw = 0.0f;

        float norm = 0.0f;
        if (range[i] > 0.0001f)
            norm = (raw - min_vals[i]) / range[i];

        if (norm < 0.0f) norm = 0.0f;
        if (norm > 1.0f) norm = 1.0f;

        sensor_values[i] = norm;
    }
}
int lost_frames = 0;
const int LOST_THRESHOLD = 20;  // tune this (5–15)
/*======================== ERROR ========================*/
float calculate_error() {
    float weights[6] = {3, 2, 1, -1, -2, -3};
    float sum = 0, total = 0;
    float minv = 1, maxv = 0;

    for (int i = 0; i < 6; i++) {
        float v = sensor_values[i];
        sum += v * weights[i];
        total += v;
        if (v < minv) minv = v;
        if (v > maxv) maxv = v;
    }

    float contrast = (maxv - minv);

    if (contrast < 0.1f) {

    lost_frames++;

    // ✅ SHORT GAP → ignore it, keep last direction
    if (lost_frames < LOST_THRESHOLD) {
        return last_error;
    }

    // ❗ REAL LOST
    if (current_state == BRAKING) return last_error;

    if (centered_count >= CENTRED_THRESHOLD) {
        snap_eol_L = left_encoder.getPulses();
        snap_eol_R = right_encoder.getPulses();
        current_state = BRAKING;
    } else {
        current_state = LOST;
        lost_counter = 0;
    }

    centered_count = 0;
    return last_error;
}
lost_frames = 0;

    float error = sum / total;

    if (fabs(error) < 1.2f) { //modify this threshold
        centered_count++;
        if (centered_count > CENTRED_THRESHOLD)
            centered_count = CENTRED_THRESHOLD;
    } else {
        centered_count = 0;
    }

    return error;
}
//braking function 
void do_braking() {

    // Compute distance travelled since EOL snap
    int dL = left_encoder.getPulses() - snap_eol_L;
    int dR = right_encoder.getPulses() - snap_eol_R;

    float avg_counts = (dL + dR) * 0.5f;
    // convert encoder counts → meters
    float travelled_m = avg_counts / COUNTS_PER_M;
    // Remaining distance to stop
    float remaining_m = (STOP_DISTANCE_MM / 1000.0f) - travelled_m;
    // if we've reached or passed stop point → stop fully
    if (remaining_m <= 0.0f) {
        target_speed_L = 0.0f;
        target_speed_R = 0.0f;
        current_state  = STOPPED;
        return;
    }
    // Normalised progress (0 → 1)
    float t = remaining_m / (STOP_DISTANCE_MM / 1000.0f);
    // clamp for safety
    if (t > 1.0f) t = 1.0f;
    if (t < 0.0f) t = 0.0f;
    // Smooth braking curve (better than linear)
    // quadratic slowdown = smoother near stop!!! note the difference 
    float speed = base_speed_ms * (t * t);

    // prevent "stall creep"    
    if (speed < 0.03f) {
        speed = 0.0f;
    }
    target_speed_L = speed;
    target_speed_R = speed;
    return;
}

/*======================== LINE PID LOOP ========================*/

int incline_tick = 0;
bool incline_brake_phase = false;
void pid_control_loop() {
    read_sensors();
    error_val = calculate_error();

    if (emergency_stop || current_state == TURNING){
        return;
    }
    
    switch (current_state) {
        case FOLLOWING:
            // 1. Check for EOL FIRST. 
            // If we see a flat black surface, we don't care about the PID error.
            /*if (!eol_active && end_of_line_detected()) {
                snap_eol_L = left_encoder.getPulses();
                snap_eol_R = right_encoder.getPulses();
                current_state = BRAKING;
                eol_active = true;
                return; // Exit following immediately
            }*/

            // 2. PID Calculations
            
            Kp_L = 0.80f;
            Kp_R = 0.80f;
            Ki   = 0.0f;
            Kd   = 1.1f;
            integral   += error_val;
            derivative  = error_val - last_error;
            correction  = (error_val  * line_Kp) + (derivative * line_Kd);
            last_error  = error_val;

            target_speed_L = base_speed_ms + correction;
            target_speed_R = base_speed_ms - correction;
            prev_state = FOLLOWING;
            break;

        case LOST: {
            lost_counter++;

            float total_signal = 0.0f;
            float minv = 1.0f;
            float maxv = 0.0f;

            for (int i = 0; i < 6; i++) {
                float v = sensor_values[i];
                total_signal += v;
                if (v < minv) minv = v;
                if (v > maxv) maxv = v;
            }

            float variance = fabs(maxv - minv);

            bool line_found =
                (total_signal > 0.12f) &&
                (variance > 0.12f) && ((sensor_values[2] > 0.25f) || (sensor_values[3] > 0.25f));

            if (line_found) {
                lost_counter = 0;

                if (prev_state == FOLLOWING) {
                    current_state = FOLLOWING;
                }
                else if (prev_state == BOOST) {
                    current_state = BOOST;
                }
                else if (prev_state == SLOW) {
                    current_state = SLOW;
                }
                else if (prev_state == INCLINE) {
                    current_state = INCLINE;
                }
                else {
                    current_state = FOLLOWING;
                }

                break;
            }
            
            
            float search_turn = 0.0f;
            float forward = 0.0f;
            
            if (prev_state == INCLINE){ 
                    current_state = INCLINE;    
                    search_turn = 0.1f;
                    forward = 0.0f;
            } else{
                search_turn = 0.1f;
                forward = 0.08f;
            }

            if (last_error > 0) {
                    target_speed_L = forward + search_turn;
                    target_speed_R = forward - search_turn;
                }
             else {
                    target_speed_L = forward - search_turn;
                    target_speed_R = forward + search_turn;
                
            }

            if (lost_counter > 150) current_state = STOPPED;
            break;
        }

        case BOOST: {
            //float line_Kp = 1.62;
            /*
            float Kp_L = 0.70f;
            float Kp_R = 0.70f;
            float Ki   = 0.0f;
            float Kd   = 0.9f;
            */
            Kp_L = 0.82f;
            Kp_R = 0.82f;
            Kd = 0.9f;
            float boost_Kp = 1.4;
            float boost_Kd = 2.2;
            integral   += error_val;
            derivative  = error_val - last_error;
            correction  = (error_val  * boost_Kp) + (derivative * boost_Kp);
            last_error  = error_val;

            target_speed_L = 1.8 + correction;
            target_speed_R = 1.8 - correction;
            prev_state = BOOST;
                break;
        }

        case SLOW: {
            /*
            float Kp_L = 0.70f;
            float Kp_R = 0.70f;
            float Ki   = 0.0f;
            float Kd   = 0.9f;
            */
            Kp_L = 0.3f;
            Kp_R = 0.3f;
            Kd = 0.5f;
            float slow_Kp = 0.2f;
            float slow_Kd = 0.55f;
            integral   += error_val;
            derivative  = error_val - last_error;
            correction  = (error_val * slow_Kp) + (derivative * slow_Kd);
            last_error  = error_val;

            target_speed_L = 0.40f + correction;
            target_speed_R = 0.40f - correction;

            prev_state = SLOW;
            break;
}

    case INCLINE: {
            prev_state = INCLINE;

            Kp_L = 0.3f;
            Kp_R = 0.3f;
            Kd = 0.5f;

            float incline_Kp = 0.2f;
            float incline_Kd = 0.5f;

            integral   += error_val;
            derivative  = error_val - last_error;
            correction  = (error_val * incline_Kp) + (derivative * incline_Kd);
            last_error  = error_val;

            incline_tick++;

            // line_ticker is 0.005s, so:
            // 300 ticks = 1.5 sec forward
            // 60 ticks  = 0.3 sec backward/brake
            if (incline_tick < 300) {
                incline_brake_phase = false;
                target_speed_L = 0.3f + correction;
                target_speed_R = 0.3f - correction;
            }
            else if (incline_tick < 360) {
                incline_brake_phase = true;
                target_speed_L = -0.2f + correction;
                target_speed_R = -0.2f - correction;
            }
            else {
                incline_tick = 0;
    }

    break;
}


        case STOPPED:
            target_speed_L = 0.0f;
            target_speed_R = 0.0f;
            break;

        case BRAKING:
            do_braking();
            break;

        default:
            current_state =STOPPED;
            break;
    }}


/*======================== SPEED TICK ========================*/
void speed_tick() {
    if (emergency_stop || current_state == STOPPED) {
        stop_motors();
        return;
    }

    if(current_state == TURNING){
        return;
    }
    const int now_left  = left_encoder.getPulses();
    const int now_right = right_encoder.getPulses();

    dL_ticks = now_left  - last_tick_left;
    dR_ticks = now_right - last_tick_right;

    last_tick_left  = now_left;
    last_tick_right = now_right;

    tickRateL = (float)dL_ticks / SAMPLE_TIME;
    tickRateR = (float)dR_ticks / SAMPLE_TIME;

    vL = tickRateToVel(tickRateL, COUNTS_PER_M);
    vR = tickRateToVel(tickRateR, COUNTS_PER_M);

    eL = target_speed_L - vL;
    eR = target_speed_R - vR;

/*
    uL = uL_prev - (eL * Kp_L)-(eL - eL_prev) * Kd; 
    uR = uR_prev - (eR * Kp_R)-(eR - eR_prev) * Kd;

    uL = EnsureSafe(uL, 0.3f, 0.8f);
    uR = EnsureSafe(uR, 0.3f, 0.8f);*/

    float duL = -(eL * Kp_L) - ((eL - eL_prev) * Kd);
    float duR = -(eR * Kp_R) - ((eR - eR_prev) * Kd);


    duL = EnsureSafe(duL, -0.4f, 0.4f);
    duR = EnsureSafe(duR, -0.4f, 0.4f);

    uL = uL_prev + duL;
    uR = uR_prev + duR;

    if (current_state == SLOW || current_state == INCLINE) {
    uL = EnsureSafe(uL, 0.20f, 0.80f);
    uR = EnsureSafe(uR, 0.20f, 0.80f);
    }
    else {
        uL = EnsureSafe(uL, 0.0f, 1.0f);
        uR = EnsureSafe(uR, 0.0f, 1.0f);
    }


    v_robot = 0.5f * (vR + vL);
    w_robot = (vR - vL) / WHEEL_BASE;

    pwmL.write(uL);
    pwmR.write(uR);

    uL_prev = uL;
    uR_prev = uR;

    eL_prev2 = eL_prev;
    eR_prev2 = eR_prev;
    eL_prev  = eL;
    eR_prev  = eR;
}

/*======================== TURN 180 (FIXED) ========================*/
void turn_180() {
    current_state = TURNING;

    // 🔴 STOP BACKGROUND CONTROL LOOPS
    speedTicker.detach();
    line_ticker.detach();

    stop_motors();
    wait_ms(1000);

    emergency_stop = false;
    
    left_encoder.reset();
    right_encoder.reset();

    float pwm_left  = 0.2f;
    float pwm_right = 0.8f;
    //bool line_found = false;

    while (!emergency_stop) {

        int leftCounts  = abs(left_encoder.getPulses());
        int rightCounts = abs(right_encoder.getPulses());
        int avg = (leftCounts + rightCounts) / 2;

        pwmL.write(pwm_left);
        pwmR.write(pwm_right);

        //line_found = ((avg >= COUNTS_180) && (sensor_values[2] > 0.25f) || (sensor_values[3] > 0.25f));

        if (avg >= COUNTS_180) {
            break;
        }

        wait_ms(5);
    }

    stop_motors();
    wait_ms(100);

    // reset controllers cleanly
    uL_prev = 0.5f;
    uR_prev = 0.5f;
    eL_prev = eR_prev = 0.0f;
    integral = 0.0f;
    last_error = 0.0f;

    centered_count = 0;
    snap_eol_L = snap_eol_R = 0;

    line_ticker.attach(&pid_control_loop, 0.009f);
    speedTicker.attach(&speed_tick, SAMPLE_TIME);

    current_state = FOLLOWING;
}

/*======================== MAIN ========================*/
int main() {
    
    /*// PWM setup
    pwmL.period(1.0f / PWM_FREQ_HZ);
    pwmR.period(1.0f / PWM_FREQ_HZ);

    dirL = 1;
    dirR = 1;
    enable = 1;*/
    
    pc.baud(9600);
    hm10.baud(9600);
    Bluetooth_power=1;


    // Compute ranges from hardcoded min/max
    for (int i = 0; i < 6; i++) {
        range[i] = max_vals[i] - min_vals[i];
    }

    // PWM setup
    pwmL.period(1.0f / PWM_FREQ_HZ);
    pwmR.period(1.0f / PWM_FREQ_HZ);

    dirL = 1;
    dirR = 1;
    enable = 1;

    
    bipR = 0;
    bipL = 0;


    left_encoder.reset();
    right_encoder.reset();

    uL_prev = 0.5f;
    uR_prev = 0.5f;
    pwmL.write(0.5f);
    pwmR.write(0.5f);

    for (int i = 0; i < 6; i++) {
        darlington[i] = 1;
    }
    //read_sensors();//run once before ticker loop, because first time all sensors are opened
    
    line_ticker.attach(&pid_control_loop, 0.005f);

    wait_ms(2000);
    speedTicker.attach(&speed_tick, SAMPLE_TIME);

    char s;
    
    while (true) {
    
    //pwmL.write(0.2);
    //pwmR.write(0.2);
        switch (current_state){
            case(FOLLOWING):{hm10.printf(" %.2f, %.2f\r\n",error_val,lost_counter);break;}
            case(LOST):{hm10.printf("LOST\r\n");break;}
            case(STOPPED):{hm10.printf("STOPPED\r\n");break;}
            case(TURNING):{hm10.printf("TURNING\r\n");break;}
            case(BRAKING):{hm10.printf("BRAKING\r\n");break;}
            case(BOOST):{hm10.printf("BOOST\r\n");break;}
            case(SLOW):{hm10.printf("SLOW\r\n");break;}
            case(INCLINE):{hm10.printf("INCLINE\r\n");break;}
        }
        if (hm10.readable()) {
            s = hm10.getc();
            pc.printf("BLE: %c\r\n", s);

            if (s == '1') {
                emergency_stop = false;
                centered_count = 0;  
                snap_eol_L = snap_eol_R = 0; 
                current_state  = FOLLOWING;
            }

            else if (s == '2') {
                emergency_stop = false;
                centered_count = 0;  
                snap_eol_L = snap_eol_R = 0; 
                current_state  = BOOST;
            }

            else if (s == '3') {
                turn_180();
                }
            else if (s == '4') {
                emergency_stop = false;
                centered_count = 0;  
                snap_eol_L = snap_eol_R = 0; 
                current_state  = SLOW;
            }

            else if (s == '5') {
                emergency_stop = false;
                centered_count = 0;
                snap_eol_L = snap_eol_R = 0;

                incline_tick = 0;
                incline_brake_phase = false;
                uL_prev = 0.5f;
                uR_prev = 0.5f;
                eL_prev = eR_prev = 0.0f;

                current_state = INCLINE;
            }

            else if (s == '0') {
                emergency_stop = true;
                current_state = STOPPED;
                stop_motors();
            }
        }
    }
}
//传感器偏置需要重新测量 Sensors zero bias need to re-test after use darlington array
//传感器红外开关时间仍需确认，防止系统过慢 replace
